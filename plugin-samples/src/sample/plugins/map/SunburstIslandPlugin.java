...
import com.bananarepublic.plugin.MapGeneratorPlugin;
...
public final class SunburstIslandPlugin implements MapGeneratorPlugin {
    @Override
    public Board generateBoard() {
        return new SunburstBoardFactory().createBoard();
    }

    private static final class SunburstBoardFactory {
        ...
        private Board createBoard() {
            ...
            attachHarbors(paths, harbors, edgeRegistry);
        ...
        private void attachHarbors(
                Map<String, Path> paths,
                Map<String, Harbor> harbors,
                Map<EdgeKey, Path> edgeRegistry
        ) {
            List<Path> coastalPaths = paths.values().stream()
            ...
            List<Harbor> harborLayout = List.of(
                    new GenericHarbor("H1", selectedPaths.get(0)),
                    new BananaHarbor("H2", selectedPaths.get(1)),
                    new GenericHarbor("H3", selectedPaths.get(2)),
                    new OreHarbor("H4", selectedPaths.get(3)),
                    new GenericHarbor("H5", selectedPaths.get(4)),
                    new WheatHarbor("H6", selectedPaths.get(5)),
                    new GenericHarbor("H7", selectedPaths.get(6)),
                    new WoodHarbor("H8", selectedPaths.get(7)),
                    new BrickHarbor("H9", selectedPaths.get(8))
            );

            for (Harbor harbor : harborLayout) {
        ...
        private List<Path> selectDistributedCoastalPaths(List<Path> coastalPaths, int count) {
            ...
            throw new IllegalStateException("Failed to distribute harbors for sample map plugin");
        }
        ...
        private record AxialCoordinate(int q, int r) {}
